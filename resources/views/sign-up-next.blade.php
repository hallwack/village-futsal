...
<div class="flex flex-col items-center justify-center w-full mt-16">
    ...
    <form class="flex flex-col items-center justify-start border border-gray-200 rounded-lg p-12" action="{{ route('register-second') }}" method="POST" enctype="multipart/form-data">
        @csrf
        <div class="flex flex-col items-center justify-start space-y-8 mb-8">
            <img src="{{ asset('img/icon/upload-icon.svg') }}" class="bg-slate-200 p-10 object-cover rounded-full" id="imgOutput" alt="upload-icon" />
            <div class="flex flex-col items-center">
                <p class="text-lg font-bold text-slate-800">Raihan Adam</p>
                <p class="text-slate-800">hallwack.id@gmail.com</p>
            </div>
            <div class="flex flex-col">
                <input type="file" name="imgInput" id="imgInput" class="file:py-2 file:px-3 file:bg-white file:border-2 file:rounded-md file:text-primary file:border-primary file:shadow-none text-slate-800" placeholder="Pilih File" />
        ...
        <a href="{{ route('login') }}" class="bg-transparent rounded w-full text-primary/60 py-2 border-2 border-primary/50 text-xl font-semibold mt-1 mb-4 text-center">Lewati</a>
    </form>
</div>
@endsection
